...
def store_stock_price(self, price_data, symbol):
    """Store stock price data via Celery"""
    try:
        from app.main import orm_app
        # Use naming convention to generate collection name
        collection_name = naming.get_price_collection_name(symbol)
        collection = orm_app.get_collection(collection_name)
        logger.info(f"Storing prices for {symbol} to {collection_name}")
