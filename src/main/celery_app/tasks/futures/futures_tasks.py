...
fetch_futures_price = shared_task(
    name="celery_app.tasks.futures.futures_tasks.fetch_futures_price",
    bind=True,
    max_retries=3
)(_fetch_func)
...
store_futures_price = shared_task(
    name="celery_app.tasks.futures.futures_tasks.store_futures_price",
    bind=True,
    max_retries=2
)(_store_func)
...
compute = shared_task(
    name="celery_app.tasks.futures.futures_tasks.compute",
    bind=True,
    max_retries=2
)(_compute_func)
...
store = shared_task(
    name="celery_app.tasks.futures.futures_tasks.store",
    bind=True,
    max_retries=2
)(_store_ind_func)