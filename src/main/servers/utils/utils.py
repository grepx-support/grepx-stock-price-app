"""MongoDB utilities for servers."""

import logging
from typing import List, Dict, Any
from price_app.main import get_collection as get_main_collection, get_database

logger = logging.getLogger(__name__)


def get_collection(collection_name: str):
    """Get a MongoDB collection."""
    return get_main_collection(None, collection_name)


def create_indexes():
    """Create indexes for collections."""
    try:
        db = get_database()
        logger.info("Creating MongoDB indexes...")
        # You can add specific index creation logic here
        logger.info("MongoDB indexes created")
    except Exception as e:
        logger.warning(f"Failed to create indexes: {e}")


def find_all(collection_name: str) -> List[Dict[str, Any]]:
    """Find all documents in a collection."""
    try:
        collection = get_collection(collection_name)
        cursor = collection.find({})
        documents = list(cursor)
        
        # Convert ObjectId to string for JSON serialization
        for doc in documents:
            if '_id' in doc:
                doc['_id'] = str(doc['_id'])
        
        return documents
    except Exception as e:
        logger.error(f"Failed to find documents in {collection_name}: {e}", exc_info=True)
        return []


def bulk_upsert(collection_name: str, records: List[Dict[str, Any]]) -> int:
    """Bulk upsert records into a collection. Uses date as the unique key."""
    if not records:
        return 0
    
    try:
        collection = get_collection(collection_name)
        inserted_count = 0
        
        for record in records:
            filter_doc = {'date': record.get('date')}
            if filter_doc['date'] is None:
                result = collection.insert_one(record)
                if result.inserted_id:
                    inserted_count += 1
            else:
                result = collection.update_one(
                    filter_doc,
                    {'$set': record},
                    upsert=True
                )
                if result.upserted_id or result.modified_count > 0:
                    inserted_count += 1
        
        logger.debug(f"Upserted {inserted_count} documents to {collection_name}")
        return inserted_count
        
    except Exception as e:
        logger.error(f"Failed to bulk upsert to {collection_name}: {e}", exc_info=True)
        return 0
